...
local ch = display.contentHeight
 ...
 local myString = "Level 2: Help Rick fix the spaceship, don't let snakes get to him"  -- Create your string
...
function jugar(self, e)
    ...
    if e.phase == "ended" then
        composer.removeScene( "introN2"  )
        composer.gotoScene( "juegoN2", {
          effect =  "fade", 
          time = 2000, 
          params = {
...
local function displayData()
      ...
      if(positionCount<=string.len(myString))then
        ...
        letterLabel = display.newText(letter,space+(positionCount*15),660+(space*row),"ARCADE_N.TTF",15) -- place the letter
        letterLabel.alpha = 0;
